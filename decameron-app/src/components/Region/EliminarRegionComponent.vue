<template>
    <div>
      ...
      <br>
      <h3 style="text-align: center;">Eliminar Región</h3>
      <h4 style="text-align: center;">Desea Eliminar el Registro?</h4>
      <br>
      <form @submit.prevent="deleteRegion" style="width:500px; margin: auto;">
        <div class="form-group">
            <p>ID: {{ region.id }}</p>
            <p>Nombre: {{ region.nombre }}</p>
            <br>
            <button type="submit" class="btn btn-primary">Eliminar</button>
            <br><br>
            <router-link to="/regiones">Regresar a la lista</router-link>
        </div>
      </form>
    </div>
  </template>
...
export default {
  ...
  methods: {
    fetchRegion() {
      axios.get(`http://localhost:3000/decameron/API/get_region.php?Id=${this.$route.params.id}`)
        .then(response => {
          this.region = response.data;
        });
    },
    deleteRegion() {
        axios.delete(`http://localhost:3000/decameron/API/delete_region.php?Id=${this.$route.params.id}`)
          .then(() => {
