<template>
    <div>
      ...
      <br>
      <h3 style="text-align: center;">Eliminar Tipo de Documento</h3>
      <h4 style="text-align: center;">Desea Eliminar el Registro?</h4>
      <br>
      <form @submit.prevent="deleteTipoDocumento" style="width:500px; margin: auto;">
        <div class="form-group">
            ...
            <br><br>
            <router-link to="/tipodocumentos">Regresar a la lista</router-link>
        </div>
      </form>
    </div>
  </template>
...
export default {
  ...
  methods: {
    fetchTipoDocumento() {
      axios.get(`http://localhost:3000/decameron/API/get_tipodocumento.php?Id=${this.$route.params.id}`)
        .then(response => {
          this.tipodocumento = response.data;
        });
    },
    deleteTipoDocumento() {
        axios.delete(`http://localhost:3000/decameron/API/delete_tipodocumento.php?Id=${this.$route.params.id}`)
          .then(() => {
            this.$router.push('/tipodocumentos');
          });
      }
