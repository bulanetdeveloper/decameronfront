<template>
  <div>
    ...
    <br>
    <h3 style="text-align: center;">Editar Tipo Documento</h3>
    <br>
    <form @submit.prevent="editarTipoDocumento" style="width:500px; margin: auto;">
  ...
  fetchTipoDocumento() {
        axios.get(`http://localhost:3000/decameron/API/get_tipodocumento.php?Id=${this.$route.params.id}`)
          .then(response => {
            ...
            this.nombre = this.tipodocumento.nombre;
          });
      },
  editarTipoDocumento() {
    axios.put('http://localhost:3000/decameron/API/update_tipodocumento.php?Nombre='+this.nombre + '&Id=' + this.$route.params.id, {
      nombre: this.nombre,
