<template>
    <div>
      ...
      <br>
      <h3 style="text-align: center;">Eliminar Estado Habitación</h3>
      <h4 style="text-align: center;">Desea Eliminar el Registro?</h4>
      <br>
      <form @submit.prevent="deleteEstadoHabitacion" style="width:500px; margin: auto;">
        <div class="form-group">
            <p>ID: {{ estadohabitacion.id }}</p>
            <p>Nombre: {{ estadohabitacion.nombre }}</p>
            <br>
            <button type="submit" class="btn btn-primary">Eliminar</button>
            <br>
            <br>
            <router-link to="/estadohabitaciones">Regresar a la lista</router-link>
        </div>
      </form>
    </div>
  </template>
...
export default {
  ...
  created() {
    this.fetchEstadoHabitacion();
  },
  methods: {
    fetchEstadoHabitacion() {
      axios.get(`http://localhost:3000/decameron/API/get_estadohabitacion.php?Id=${this.$route.params.id}`)
        .then(response => {
          this.estadohabitacion = response.data;
        });
    },
    deleteEstadoHabitacion() {
        axios.delete(`http://localhost:3000/decameron/API/delete_estadohabitacion.php?Id=${this.$route.params.id}`)
          .then(() => {
            this.$router.push('/estadohabitaciones');
          });
      }
