<template>
  <div>
    ...
    <br>
    <h3 style="text-align: center;">Editar Estado Habitación</h3>
    <form @submit.prevent="editEstadoHabitacion" style="width:500px; margin: auto;">
  ...
  fetchEstadoHabitacion() {
    axios.get(`http://localhost:3000/decameron/API/get_estadohabitacion.php?Id=${this.$route.params.id}`)
      .then(response => {
        ...
        this.nombre = this.estadohabitacion.nombre;
      });
  },
  editEstadoHabitacion() {
    axios.put(`http://localhost:3000/decameron/API/update_estadohabitacion.php?Id=${this.$route.params.id}&Nombre=${this.nombre}`, {
      nombre: this.nombre
    })
    .then(() => {
