<template>
    <div>
      ...
      <br>
      <h3 style="text-align: center;">Eliminar Habitación</h3>
      ...
      <form @submit.prevent="deleteTipoHabitacion" style="width:500px; margin: auto;">
        <div class="form-group">
            ...
            <p>N° Hab: {{ habitacion.numhab }}</p>
            <p>Piso: {{ habitacion.piso }}</p>
            <p>Tipo de Habitación: {{ habitacion.tipo }}</p>
            <p>Acomodación: {{ habitacion.acomodacion }}</p>
            <p>Hotel: {{ habitacion.hotel }}</p>
            <p>Estado: {{ habitacion.estado }}</p>
            <br>
            <button type="submit" class="btn btn-primary">Eliminar</button>
            <br>
            <br>
            <router-link to="/habitaciones">Regresar a la lista</router-link>
        </div>
    </form>
    </div>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      deleteHabitacion() {
        axios.delete(`http://localhost:3000/decameron/API/delete_habitacion.php?Id=${this.$route.params.id}`)
            .then(() => {
