<template>
    <div>
      ...
      <br>
      <h3 style="text-align: center;">Eliminar Ciudad</h3>
      <h4 style="text-align: center;">Desea Eliminar el Registro?</h4>
      <br>
      <form @submit.prevent="deleteCiudad" style="width:500px; margin: auto;">
        <div class="form-group">
            <p>ID: {{ ciudad.id }}</p>
            <p>Nombre: {{ ciudad.nombre }}</p>
            <p>Región: {{ ciudad.region }}</p>
            <br>
            <button type="submit" class="btn btn-primary">Eliminar</button>
            <br><br>
            <router-link to="/ciudades">Regresar a la lista</router-link>
        </div>
      </form>
    </div>
  </template>
...
export default {
  ...
  methods: {
    ...
    deleteCiudad() {
        axios.delete(`http://localhost:3000/decameron/API/delete_ciudad.php?Id=${this.$route.params.id}`)
          .then(() => {
