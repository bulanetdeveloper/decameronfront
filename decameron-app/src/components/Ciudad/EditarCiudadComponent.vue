<template>
    <div>
      ...
      <br>
      <h3 style="text-align: center;">Editar Ciudad</h3>
      <form @submit.prevent="editarCiudad" style="width:500px; margin: auto;">
        ...
        <div class="form-group">
          ...
          <select class="custom-select" v-model="regionId">
          <option :value="region.id" v-for="region in regiones" :key="region.id">{{ region.nombre }}</option>
        </select>
        </div>
        <br>
  ...
  export default {
    ...
    methods: {
      editarCiudad() {
        axios.put('http://localhost:3000/decameron/API/update_ciudad.php?Nombre='+ this.nombre + '&RegionId=' + this.regionId +'&Id='+ this.$route.params.id, {
          nombre: this.nombre,
      ...
      fetchCiudad() {
      axios.get(`http://localhost:3000/decameron/API/get_ciudad.php?Id=${this.$route.params.id}`)
        .then(response => {
      ...
      fetchRegiones() {
          axios.get('http://localhost:3000/decameron/API/get_all_region.php')
            .then(response => {
